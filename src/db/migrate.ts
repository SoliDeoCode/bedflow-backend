...
function seedSystemViews() {
  ...
  const ins = db.prepare(
    `INSERT OR IGNORE INTO saved_views (name, created_by, selected_wards, is_shared, is_system, created_at, updated_at)
     VALUES (?, NULL, ?, 1, 1, ?, ?)`
  );
  // Use name as de-dup key — if system view with this name already exists, skip
  const exists = db.prepare("SELECT name FROM saved_views WHERE is_system=1").all<{ name: string }>();
