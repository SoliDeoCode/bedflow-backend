...
async function run() {
  ...
  await db.transaction(async () => {
    ...
    for (const b of BLOCK_DATA) {
      await db.prepare(
        `INSERT INTO blocks (name, name_key, label, sort_order, created_at, updated_at)
         VALUES (?,?,?,?,?,?)`
      ).run(b.name, b.name.toUpperCase(), b.label, b.sortOrder, now, now);
    }
    ...
    for (const row of blockRows) blockIdFor[row.name_key] = row.id;
      ...
      for (const w of b.wards) {
        await db.prepare(
          `INSERT INTO wards (name, block_id, total_beds, created_at, updated_at)
           VALUES (?,?,?,?,?)`
        ).run(w.name, blockId, w.total, now, now);
        ...
        const insBedDetail = db.prepare(
          `INSERT INTO bed_details (ward_id, bed_number, physical_status, reservation_status, updated_at, updated_by)
    ...
    await db.prepare(
      `INSERT INTO users (username, password_hash, role, name, shift, block_id, created_at, updated_at)
       VALUES (?,?,?,?,?,NULL,?,?)`
    ).run("coo", bcrypt.hashSync("coo123", 10), "COO", "Chief Operating Officer", "morning", now, now);
    ...
    for (const v of SYSTEM_VIEWS) {
      ...
      ).run(v.name, JSON.stringify(v.wards), now, now);
    }
