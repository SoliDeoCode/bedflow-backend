...
function run() {
  ...
  db.transaction(() => {
    ...
    const insBlock = db.prepare(
      `INSERT OR IGNORE INTO blocks (name, name_key, label, sort_order, created_at, updated_at)
       VALUES (?,?,?,?,?,?)`
    );
    BLOCK_DATA.forEach((b, i) => insBlock.run(b.name, b.name.toUpperCase(), b.label, i + 1, now, now));

    const blockIdFor: Record<string, number> = {};
    for (const b of db.prepare("SELECT id, name_key FROM blocks").all<{ id: number; name_key: string }>())
      blockIdFor[b.name_key] = b.id;

    // ── 2. Wards + bed rows ───────────────────────────────────────────────────
    const insWard = db.prepare(
      `INSERT OR IGNORE INTO wards (name, block_id, total_beds, created_at, updated_at)
       VALUES (?,?,?,?,?)`
    );
    const insBed = db.prepare(
      `INSERT OR IGNORE INTO beds (ward_id, total, vacant, reserved, occupied, updated_at)
    ...
    for (const a of PRE_ACCOUNTS) {
      ...
      insUser.run(a.username, bcrypt.hashSync(`${a.username}123`, 10),
                  "PRE", displayName, "morning", bid, now, now);
    }
    ...
    const insShift = db.prepare(
      "INSERT OR IGNORE INTO shifts (key, label, start_time, end_time) VALUES (?,?,?,?)"
    );
