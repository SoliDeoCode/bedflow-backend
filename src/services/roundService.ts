...
import {
  inShift, currentRound, roundKey, todayStr, minsNow, WARDS, type ShiftKey,
...
export function alarmState(preCode: string, shift: ShiftKey) {
  const mins = minsNow();
...
export function submitRound(preCode: string, userId: number) {
  const wards = wardsForPre(preCode);
  if (wards.length === 0) throw new HttpError(400, "No wards to submit");
  for (const w of wards) {
    if (w.vacant === null) throw new HttpError(400, `Enter all wards first (${w.ward} missing)`);
  ...
  try {
    db.prepare(
      "INSERT INTO pre_rounds (pre_code, user_id, shift, round_key, start_min, submitted_at, snapshot) VALUES (?,?,?,?,?,?,?)"
    ).run(preCode, userId, shift, key, round.startMin, Date.now(), JSON.stringify(wards));
