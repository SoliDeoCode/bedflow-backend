...
router.get("/wards", asyncH(async (_req, res) => {
  const wards = db.prepare(
    `SELECT w.id, w.name, w.pre_code, w.total_beds,
            b.vacant, b.reserved, b.occupied, f.name AS floor
     FROM wards w JOIN beds b ON b.ward_id = w.id
     LEFT JOIN floors f ON f.id = w.floor_id ORDER BY w.pre_code, w.name`
  ).all();
  res.json({ wards });
}));
...
router.get("/users", asyncH(async (_req, res) => {
  const users = db.prepare(
    `SELECT u.id, u.username, u.role, u.name, u.shift, a.pre_code AS pre
