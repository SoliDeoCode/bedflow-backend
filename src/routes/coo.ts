  ...
  const floors = base.floors.map((f) => ({
    ...
    pres: f.pres.map((p) => {
      const uid = db.prepare(
        "SELECT u.id FROM users u JOIN pre_assignments a ON a.user_id=u.id WHERE a.pre_code=?"
      ).get<{ id: number }>(p.pre);
...
router.get("/compliance", asyncH(async (_req, res) => {
  ...
  const rows = Object.keys(WARDS).filter((pre) => WARDS[pre].length > 0).map((pre) => {
    const uid = db.prepare(
      "SELECT u.id, u.shift, u.name FROM users u JOIN pre_assignments a ON a.user_id=u.id WHERE a.pre_code=?"
    ).get<{ id: number; shift: keyof typeof SHIFTS; name: string }>(pre);
    ...
    const expected = Math.max(0, Math.min(Math.floor(elapsed / PRE_INTERVAL_MIN) + 1,
      Math.floor((hmToMin(s.end) - start + 1440) % 1440 / PRE_INTERVAL_MIN)));
    const submitted = db.prepare(
      "SELECT COUNT(*) AS c FROM pre_rounds WHERE pre_code=? AND round_key LIKE ?"
    ).get<{ c: number }>(pre, `%|${today}|%`)?.c ?? 0;
