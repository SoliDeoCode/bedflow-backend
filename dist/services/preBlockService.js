...
export async function getPreBlock(id) {
...
export async function createPreBlock(opts) {
    ...
    const clash = await db.prepare("SELECT id FROM pre_blocks WHERE LOWER(name)=LOWER(?)")
        .get(opts.name.trim());
    if (clash)
        throw new HttpError(409, `PRE Block "${opts.name}" already exists`);
    const t = now();
    let blockId = 0;
    await db.transaction(async () => {
        const r = await db.prepare(`INSERT INTO pre_blocks (name, description, status, created_by, created_at, updated_at)
       VALUES (?,?,?,?,?,?) RETURNING id`).run(opts.name.trim(), opts.description?.trim() || null, "active", opts.managerId, t, t);
        blockId = Number(r.lastInsertRowid);
        for (const wardId of opts.wardIds) {
            const ward = await db.prepare("SELECT id FROM wards WHERE id=?").get(wardId);
            if (!ward)
                throw new HttpError(404, `Ward ${wardId} not found`);
            await db.prepare("INSERT INTO pre_block_wards (pre_block_id, ward_id, created_at) VALUES (?,?,?)").run(blockId, wardId, t);
        }
    });
...
export async function editPreBlock(opts) {
    ...
    await db.transaction(async () => {
        if (opts.name !== undefined) {
            const clash = await db.prepare("SELECT id FROM pre_blocks WHERE LOWER(name)=LOWER(?) AND id<>?").get(opts.name.trim(), opts.blockId);
            if (clash)
                throw new HttpError(409, `PRE Block "${opts.name}" already exists`);
        ...
        if (opts.wardIds !== undefined) {
            ...
            for (const wardId of opts.wardIds) {
                ...
                if (!ward)
                    throw new HttpError(404, `Ward ${wardId} not found`);
                await db.prepare("INSERT INTO pre_block_wards (pre_block_id, ward_id, created_at) VALUES (?,?,?)").run(opts.blockId, wardId, t);
            }
...
export async function setPreBlockStatus(blockId, status, managerId) {
...
export async function deletePreBlock(blockId, managerId) {
    const block = await db.prepare("SELECT name FROM pre_blocks WHERE id=?")
        .get(blockId);
    if (!block)
        throw new HttpError(404, "PRE Block not found");
