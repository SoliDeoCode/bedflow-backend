...
export async function _recalcWardTotals(wardId) {
  ...
  `).get(wardId);
    ...
    const now = Date.now();
    await db.prepare("UPDATE beds SET total=?, vacant=?, reserved=?, occupied=?, updated_at=? WHERE ward_id=?").run(total, counts.vacant || 0, counts.reserved || 0, counts.occupied || 0, now, wardId);
...
export async function generateBeds(opts) {
    ...
    await db.transaction(async () => {
        for (let i = 0; i < opts.count; i++) {
            ...
            const r = await db.prepare("INSERT INTO bed_details (ward_id, bed_number, status, updated_at, updated_by) VALUES (?,?,?,?,?) ON CONFLICT (ward_id, bed_number) DO NOTHING").run(opts.wardId, String(opts.startNumber + i), "VACANT", now, opts.userId);
            inserted += r.changes;
        }
...
export async function addSingleBed(opts) {
    ...
    await db.transaction(async () => {
        ...
        const r = await db.prepare("INSERT INTO bed_details (ward_id, bed_number, status, updated_at, updated_by) VALUES (?,?,?,?,?) RETURNING id").run(opts.wardId, trimmed, "VACANT", now, opts.userId);
...
export async function listBeds(wardId, status) {
...
export async function updateBedStatus(opts) {
    const bed = await db.prepare("SELECT id, ward_id, status FROM bed_details WHERE id=?").get(opts.bedId);
    if (!bed)
        throw new HttpError(404, "Bed not found");
    const valid = ["VACANT", "RESERVED", "OCCUPIED"];
    ...
    const now = Date.now();
    await db.transaction(async () => {
        await db.prepare("UPDATE bed_details SET status=?, updated_at=?, updated_by=? WHERE id=?").run(opts.newStatus, now, opts.userId, opts.bedId);
        await db.prepare("INSERT INTO bed_movements (bed_id, old_status, new_status, changed_by, changed_at) VALUES (?,?,?,?,?)").run(opts.bedId, bed.status, opts.newStatus, opts.userId, now);
        await _recalcWardTotals(bed.ward_id);
    });
    await audit(opts.userId, "bed_status_update", String(opts.bedId), { old: bed.status, new: opts.newStatus, wardId: bed.ward_id });
    return { ok: true, status: opts.newStatus };
}
