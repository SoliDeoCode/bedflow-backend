...
export async function audit(userId, action, entity, detail) {
    ...
    await db.prepare("INSERT INTO audit_logs (ts, user_id, action, entity, detail) VALUES (?,?,?,?,?)").run(Date.now(), userId, action, entity, json);
}
