...
async function myPreBlock(req) {
    ...
    const block = await db.prepare("SELECT id, name FROM pre_blocks WHERE id=?").get(preBlockId);
    if (!block)
        throw new HttpError(400, "Assigned PRE Block not found");
    return block;
}
...
router.post("/ward", asyncH(async (req, res) => {
    ...
    const owns = await db.prepare("SELECT 1 FROM pre_block_wards WHERE pre_block_id=? AND ward_id=?").get(block.id, wardId);
    if (!owns)
...
router.patch("/beds/:id/status", asyncH(async (req, res) => {
    ...
    const owns = await db.prepare(`SELECT bd.id FROM bed_details bd
     ...
     WHERE bd.id = ? AND pbw.pre_block_id = ?`).get(bedId, block.id);
    if (!owns)
        throw new HttpError(403, "Bed not in your PRE Block");
    ...
    const stationRow = await db.prepare("SELECT station_id FROM wards WHERE id=?").get(result.ward_id);
