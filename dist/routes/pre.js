...
function myPre(req) {
    ...
    if (!pre)
        throw new HttpError(400, "No PRE assignment");
    return pre;
}
router.get("/me", asyncH(async (req, res) => {
    ...
    const fl = db.prepare("SELECT f.name AS floor FROM wards w LEFT JOIN floors f ON f.id=w.floor_id WHERE w.pre_code=? LIMIT 1").get(pre);
    res.json({
...
router.post("/ward", asyncH(async (req, res) => {
    ...
    if (!owns)
        throw new HttpError(403, "Ward not assigned to you");
