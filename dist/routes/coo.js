...
router.get("/compliance", asyncH(async (_req, res) => {
    ...
    const blocks = await db.prepare(`SELECT b.id, b.name,
    ...
    const rows = await Promise.all(blocks.map(async (block) => {
        ...
        const submittedRow = await db.prepare("SELECT COUNT(*) AS c FROM pre_rounds WHERE block_id=? AND submitted_at>=?").get(block.id, startOfDayIST());
