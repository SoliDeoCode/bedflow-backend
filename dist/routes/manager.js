...
router.post("/blocks", asyncH(async (req, res) => {
    ...
    res.status(201).json(createBlock({ name, label, managerId: req.user.id }));
}));
router.put("/blocks/:id", asyncH(async (req, res) => {
    ...
    res.json(editBlock({ blockId: Number(req.params.id), name, label: label ?? undefined, sortOrder, managerId: req.user.id }));
}));
router.delete("/blocks/:id", asyncH(async (req, res) => {
    res.json(deleteBlock(Number(req.params.id), req.user.id));
}));
// ── wards ─────────────────────────────────────────────────────────────────────
router.get("/wards", asyncH(async (_req, res) => {
    const wards = db.prepare(`SELECT w.id, w.name, w.block_id, w.total_beds,
...
router.post("/wards", asyncH(async (req, res) => {
    ...
    res.status(201).json(createWard({ name, blockId, totalBeds, managerId: req.user.id }));
}));
router.put("/wards/:id", asyncH(async (req, res) => {
    ...
    res.json(editWard({ wardId: Number(req.params.id), name, totalBeds, blockId, managerId: req.user.id }));
}));
...
router.get("/users", asyncH(async (_req, res) => {
    const users = db.prepare(`SELECT u.id, u.username, u.role, u.name, u.shift,
...
router.post("/pre", asyncH(async (req, res) => {
    ...
    res.status(201).json(createPre({ ...b, managerId: req.user.id }));
}));
...
router.post("/wards/:id/generate-beds", asyncH(async (req, res) => {
    ...
    res.status(201).json(generateBeds({ wardId: Number(req.params.id), startNumber, count, userId: req.user.id }));
}));
router.post("/wards/:id/beds", asyncH(async (req, res) => {
    const { bedNumber } = z.object({ bedNumber: z.string().min(1) }).parse(req.body);
    res.status(201).json(addSingleBed({ wardId: Number(req.params.id), bedNumber, userId: req.user.id }));
}));
...
router.get("/history", asyncH(async (req, res) => {
    ...
    res.json({ rounds: historyForDate(date, blockId) });
}));
