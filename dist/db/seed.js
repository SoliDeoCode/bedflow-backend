...
import { FLOOR_MAP, WARDS, SHIFTS, PRE_INTERVAL_MIN, SEED_ACCOUNTS, } from "../config/domain.js";
function run() {
    migrate();
    const now = Date.now();
    db.transaction(() => {
        ...
        for (const name of Object.keys(FLOOR_MAP))
            insFloor.run(name);
        const floorId = {};
        for (const f of db.prepare("SELECT id, name FROM floors").all())
            floorId[f.name] = f.id;
        // Wards + beds
        const insWard = db.prepare("INSERT OR IGNORE INTO wards (name, floor_id, pre_code, total_beds, created_at, updated_at) VALUES (?,?,?,?,?,?)");
        const insBed = db.prepare("INSERT OR IGNORE INTO beds (ward_id, total, vacant, reserved, occupied, updated_at) VALUES (?,?,NULL,NULL,NULL,NULL)");
        ...
        // Users
        const insUser = db.prepare("INSERT OR IGNORE INTO users (username, password_hash, role, name, shift, created_at, updated_at) VALUES (?,?,?,?,?,?,?)");
        const insAssign = db.prepare("INSERT OR IGNORE INTO pre_assignments (user_id, pre_code, created_at) VALUES (?,?,?)");
        for (const a of SEED_ACCOUNTS) {
            insUser.run(a.username, bcrypt.hashSync(a.password, 10), a.role, a.name, "morning", now, now);
            if (a.pre) {
                const u = db.prepare("SELECT id FROM users WHERE username = ?").get(a.username);
                if (u)
                    insAssign.run(u.id, a.pre, now);
            }
        }
        // Shifts
