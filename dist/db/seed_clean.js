...
async function run() {
  ...
  `);
    ...
    for (const u of users) {
        ...
        await db.prepare(`INSERT INTO users (username, password_hash, role, name, shift, created_at, updated_at)
       VALUES (?, ?, ?, ?, 'morning', ?, ?)`).run(u.username, hash, u.role, u.name, now, now);
        console.log(`  Created: ${u.username.padEnd(12)} (${u.role})`);
    }
    // Shifts (required for app to function)
    await db.prepare("INSERT INTO shifts (key, label, start_time, end_time) VALUES (?,?,?,?)").run("morning", "Morning", "07:00", "14:00");
    await db.prepare("INSERT INTO shifts (key, label, start_time, end_time) VALUES (?,?,?,?)").run("evening", "Evening", "14:00", "21:00");
    await db.prepare("INSERT INTO shifts (key, label, start_time, end_time) VALUES (?,?,?,?)").run("night", "Night", "21:00", "07:00");
